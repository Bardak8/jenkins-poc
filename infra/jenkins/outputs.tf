output "cluster_id" {
  description = "ID du cluster Kapsule référencé (créé manuellement, non géré par ce Terraform)."
  value       = data.scaleway_k8s_cluster.poc.id
}

output "kubeconfig" {
  description = "Kubeconfig du cluster (sensible : ne jamais committer une sortie de cette valeur)."
